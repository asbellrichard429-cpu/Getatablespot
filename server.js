...
//   STRIPE_PRICE_GROWTH/PREMIUM/DINER
//   STRIPE_LINK_GROWTH/PREMIUM/DINER
...
// growth  = $49/mo (was "basic" $49)
// premium = $149/mo (was "pro" $99; elite $199/$299 removed)
// diner   = $2.99/mo (Pro diner subscription, unchanged)
const STRIPE_PRICES = {
...
const PLAN_INFO = {
  starter: { label: 'Starter', price: 'Free',       trial: 0 },
  growth:  { label: 'Growth',  price: '$49/month',  trial: 7 },
  premium: { label: 'Premium', price: '$149/month', trial: 7 },
...
function buildVenue(gp, uLat, uLng){
  ...
  return{
    ...
    isFeatured:false, plan:'starter', tonightMsg:null, emoji:'🍽️',
...
app.post('/api/auth/register', async(req,res)=>{
  try{
    ...
    if(restaurantId) await db.upsertRestaurant({googlePlaceId:restaurantId,name:restaurantName||'My Restaurant',ownerId:owner.id,plan:plan||'starter'});
...
app.post('/api/restaurant-claim', async(req,res)=>{
  try{
    ...
    const pi = PLAN_INFO[plan] || PLAN_INFO.growth;
    const isFreePlan = plan === 'starter';

    // ── Free (Starter) plan — no Stripe, just register and activate ──
    if(isFreePlan){
      // Upsert the restaurant as starter plan
      try{
        await db.upsertRestaurant({
          googlePlaceId: venueId || `manual-${Date.now()}`,
          name: restaurantName,
          plan: 'starter',
          is_featured: false,
        });
      }catch(e){ console.warn('DB upsert (non-fatal):', e.message); }

      // Notify us
      await sendEmail({
        to: NOTIFY_EMAIL,
        subject: `🍽️ New Free Claim — ${restaurantName}`,
        html: `<div style="font-family:sans-serif;padding:24px"><h2>New Starter Claim</h2><p><strong>${restaurantName}</strong> · ${restaurantAddress||''}</p><p>${ownerName||'Unknown'} · ${role||'Owner'} · ${email} · ${phone||''}</p><p style="color:#2E9E5B;font-weight:700">Starter Plan — Free</p></div>`
      });

      // Welcome email to restaurant — direct to auth/dashboard
      await sendEmail({
        to: email,
        subject: `Welcome to GetATableSpot — ${restaurantName} is live! 🎉`,
        html: `<div style="font-family:sans-serif;max-width:520px;margin:0 auto;padding:32px 24px"><h2 style="font-family:Georgia,serif">You're live, ${(ownerName||'there').split(' ')[0]}! 🎉</h2><p style="color:#6B7A8D;line-height:1.65"><strong>${restaurantName}</strong> is now on GetATableSpot. Diners searching for short waits near you can discover your restaurant right now.</p><p style="color:#6B7A8D;line-height:1.65;margin-top:12px">Create your dashboard login to start updating your wait time and managing your listing.</p><div style="text-align:center;margin:24px 0"><a href="${APP_URL}/restaurant-auth.html?email=${encodeURIComponent(email)}&placeId=${encodeURIComponent(venueId||'')}&restaurantName=${encodeURIComponent(restaurantName)}&plan=starter" style="display:inline-block;background:#E8430A;color:white;font-weight:700;font-size:.9rem;padding:13px 28px;border-radius:6px;text-decoration:none">Set Up My Dashboard →</a><p style="font-size:.72rem;color:#6B7A8D;margin-top:8px">Free forever · Upgrade to Growth anytime for 2–3× more visibility</p></div><p style="font-size:.78rem;color:#6B7A8D;text-align:center">— Richard & Stephanie, GetATableSpot</p></div>`
      });

      return res.json({ success: true, stripeLink: null, plan: 'starter' });
    }

    // ── Paid plans (growth / premium) — Stripe checkout ──
    let stripeLink = '';
    ...
    if(priceId && !process.env.STRIPE_SECRET_KEY?.includes('placeholder')){
      try{
        const s = await stripe.checkout.sessions.create({
          ...
          subscription_data: {
            trial_period_days: pi.trial,
            metadata: { restaurantName, ownerName, plan, venueId: venueId||'' }
          },
          customer_email: email,
          success_url: `${APP_URL}/restaurant-dashboard.html?session={CHECKOUT_SESSION_ID}&plan=${plan}`,
          cancel_url: `${APP_URL}/claim-restaurant.html?cancelled=1`,
          metadata: { restaurantName, ownerName, email, phone, role, plan, venueId: venueId||'' }
        });
        stripeLink = s.url;
      }catch(e){ console.error('Stripe:', e.message); }
    }

    // Fallback Stripe payment links
    if(!stripeLink){
      const fallback = {
        growth:  process.env.STRIPE_LINK_GROWTH,
        premium: process.env.STRIPE_LINK_PREMIUM,
      };
      stripeLink = fallback[plan] || `${APP_URL}/restaurant-auth.html`;
    }

    // Notify us
    await sendEmail({
      to: NOTIFY_EMAIL,
      subject: `🍽️ New Claim — ${restaurantName} (${pi.label})`,
      html: `<div style="font-family:sans-serif;padding:24px"><h2>New Restaurant Claim</h2><p><strong>${restaurantName}</strong> · ${restaurantAddress||''}</p><p>${ownerName} · ${role||'Owner'} · ${email} · ${phone||''}</p><p style="color:#E8430A;font-weight:700">${pi.label} — ${pi.price}</p><a href="${stripeLink}">${stripeLink}</a></div>`
    });

    // Welcome email with Stripe link
    await sendEmail({
      to: email,
      subject: `Welcome to GetATableSpot — ${restaurantName} is being activated 🎉`,
      html: `<div style="font-family:sans-serif;max-width:520px;margin:0 auto;padding:32px 24px"><h2 style="font-family:Georgia,serif">Welcome, ${(ownerName||'there').split(' ')[0]}! 🎉</h2><p style="color:#6B7A8D;line-height:1.65">Your restaurant <strong>${restaurantName}</strong> is being activated on the <strong>${pi.label}</strong> plan.</p><div style="text-align:center;margin:20px 0"><a href="${stripeLink}" style="display:inline-block;background:#E8430A;color:white;font-weight:700;font-size:.9rem;padding:13px 28px;border-radius:6px;text-decoration:none">Complete Setup → Start Free 7 Days</a><p style="font-size:.72rem;color:#6B7A8D;margin-top:8px">${pi.trial} days free · Cancel anytime · ${pi.price} after trial</p></div><p style="font-size:.78rem;color:#6B7A8D;text-align:center">— Richard & Stephanie, GetATableSpot</p></div>`
    });
...
async function handleStripeWebhook(req,res){
  ...
  if(event.type==='checkout.session.completed'){
    ...
    if(venueId){
      // growth and premium both get featured placement
      const isFeatured = ['growth','premium'].includes(plan);
      await db.upsertRestaurant({googlePlaceId:venueId, name:restaurantName||'Restaurant', plan}).catch(console.error);
      await db.updateRestaurant(venueId,{
        plan,
  ...
  if(event.type==='customer.subscription.deleted'){
    ...
    if(rows[0]){
      // Downgrade to starter (free) rather than deleting
      await db.updateRestaurant(rows[0].google_place_id,{plan:'starter',is_featured:false}).catch(console.error);
      cache.flushAll();
    }
  }
    ...
    if(STRIPE_PRICES.diner&&!process.env.STRIPE_SECRET_KEY?.includes('placeholder')){
      try{
        const s=await stripe.checkout.sessions.create({
          ...
          line_items:[{price:STRIPE_PRICES.diner,quantity:1}],
          subscription_data:{trial_period_days:7},
          customer_email:email,
          success_url:`${APP_URL}/index.html?pro=1`,
          cancel_url:`${APP_URL}/pro-subscription.html`
        });
        checkoutUrl=s.url;
